...
card.innerHTML = `<button class="check-btn" aria-label="Отметить выполнение сегодня" aria-pressed="${doneToday}"> ${checkIconSvg()} </button> <div class="habit-info"> <div class="habit-name-row"> <span class="habit-name">${escapeHtml(habit.name)}</span> </div> <div class="habit-meta"> <span class="habit-created">${formatCreatedDate(habit.createdAt)}</span> <span class="habit-streak"><span class="flame">🔥</span>${streak} ${streakWord}</span> </div> <div class="habit-progress" title="Последние 7 дней">${dotsHtml}</div> </div> <div class="habit-actions"> <button class="icon-btn edit-btn" aria-label="Переименовать">✎</button> <button class="icon-btn delete-btn" aria-label="Удалить">🗑</button> </div>`;
return card;
}
...
actions.innerHTML = `<div class="confirm-row"> Удалить? <button class="confirm-btn confirm-yes">Да</button> <button class="confirm-btn confirm-no">Нет</button> </div>`;
...
/* ---------- bottom nav: скрытие при скролле вниз ---------- */
...
if (delta > 6 && y > 140) {
// листаем вниз — прячем переключатель
pageNav.classList.add('nav-hidden');
lastScrollY = y;
} else if (delta < -6) {
// листаем вверх — возвращаем
